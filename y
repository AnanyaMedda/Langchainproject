...
const WebIngester = ({ className }: { className?: string }) => {
  ...
  const { readyState, sendJsonMessage } = useWebSocket(WS_URL, {
    ...
    onMessage: (event: MessageEvent<unknown>) => {
      try {
        // Safely parse and assert properties without implicitly relying on "any"
        const rawMessage = typeof event.data === 'string' ? (JSON.parse(event.data) as unknown) : event.data;
        
        if (rawMessage && typeof rawMessage === 'object' && 'url' in rawMessage) {
          const message = rawMessage as Record<string, unknown>;
          
          if (typeof message.url === 'string') {
            const urlStr = message.url;
            const resultStr = typeof message.result === 'string' ? message.result : '';
  ...
  const onIngest = () => {
    ...
    // Type-safe mapping guaranteed to output strings only
    const validResults = results
